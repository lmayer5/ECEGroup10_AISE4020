...
def get_obstacles():
    ...
    #Bottom left: Bedroom layout (obstacles spaced out)
    obs_list = [
        (pygame.Rect(30, 350, 150, 80), (200, 0, 0)),  # bed (red)
        (pygame.Rect(250, 350, 80, 50), (200, 0, 0)),  # dresser (red)
        (pygame.Rect(30, 500, 40, 40), (200, 0, 0)),   # nightstand (red)
        (pygame.Rect(150, 500, 60, 40), (200, 0, 0))   # table (red)
    ]
    for rect, color in obs_list:
        obstacles.append(rect_to_obstacle(rect, color))
    
    #Add Walls Around the Environment
    env_left = 0
    env_top = 0
    env_right = 900
    env_bottom = 600
...
def simulation(mode):
    #Initialize Pygame with an OpenGL display
    pygame.init()
    ...
    while running:
        ...
        if mode == "head":
            ...
            # i/l for forward/backward, p/k for turning
            if keys[pygame.K_i]:
                forward_speed = SPEED_SCALE
        ...
        # Fine turn adjustments using z and x
        if keys[pygame.K_z]:
        ...
        #Compute wheelchair center (its y is half its height)
        ...
        camera_pos = ( front_face[0] + math.cos(angle)*extra_offset,
                       front_face[1] + 5,  # raise the camera a bit
                       front_face[2] + math.sin(angle)*extra_offset )
        # Look one unit ahead of the camera
...
def main():
    mode = main_menu()  #Show the start menu and choose control mode.
    simulation(mode)    #Run the 3D simulation with the chosen mode.
